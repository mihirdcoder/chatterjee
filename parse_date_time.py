...
re_dmy = '(' + "|".join(day_variations + minute_variations + year_variations + week_variations + month_variations + hour_variations) + ')'
re_duration = '(before|after|earlier|later|in|ago|from\snow)'
...
regex = [
    ...
    (re.compile(
        ...
        (
            ...
            (?P<unit>%s)s?\s # Matches days, months, years, weeks, hours, minutes
    ...
    (re.compile(
        ...
        (
            (?P<duration>%s)\s # in
            ((?P<number>\d+|(%s[-\s]?)+)\s)? # Matches any number or string 25 or twenty five
            (?P<unit>%s)s? # Matches days, months, years, weeks, hours, minutes
            s(\s*(?P<base_time>(%s)))?
            ((\s|,\s|\s(%s))?\s*(%s))?
        )
        '''% ( re_duration, numbers, re_dmy, day_nearest_names, re_separator, re_time),
        (re.VERBOSE | re.IGNORECASE)
        ),
        lambda m, base_date: dateFromDuration(
            base_date,
...
def dateFromDuration(base_date, numberAsString, unit, duration, base_time = None):
    ...
    elif duration == 'after' or duration == 'later' or duration == 'from now' or duration == 'in':
        if ('years' in args):
            return datetime(base_date.year + args['years'], base_date.month, base_date.day)
        return base_date + timedelta(**args)
...
# Parses date
def datetime_parser(text, base_date = datetime.now()):
    matches = []
    found_array = []
...
#keyword, dt, _ = datetime_parsing('set an alarm for 20:22 tomorrow.')[0]

##print (datetime_parser('set an alarm for 20:22 tomorrow.'))
####
##print (datetime_parser('set an alarm tomorrow at 12:00am.'))
##
##print (datetime_parser('remind me in 2 minutes'))
##
##print (datetime_parser('remind me to drink 2 minutes after 4:00pm'))
